...
struct reordering_type {
  enum { NONE, AMD, RCM, MC64 };
};

struct equilibration_type {
  enum { NONE, BUNCH, RUIZ, MC64 };
};
...
/*!	\brief Saves a permutation vector vec as a permutation matrix in matrix
   market (.mtx) format. \param vec the permutation vector. \param filename the
   filename the matrix will be saved under.
*/
...
/*!	\brief Reads in a dense row or column vector vec in matrix market (.mtx)
   format. \param rhs the permutation vector. \param filename the filename the
   matrix will be saved under.
*/
template <class el_type>
...
public:
  ...
  bool has_rhs; ///< Set to true if we have a right hand side that we expect to
                ///< solve.
  bool perform_inplace; ///< Set to true if we are factoring the matrix A
                        ///< inplace.
  ...
  */
   ...
   */
  void set_inplace(bool inplace) { perform_inplace = inplace; }
  ...
  /*! \brief Factors the matrix A into P' * S * A * S * P = LDL' in addition to
     ...
     pivoting. \param max_iter the maximum number of iterations for minres
     (ignored if no right hand side).
  */
  void solve(double fill_factor, double tol, double pp_tol, int max_iter = -1,
             double minres_tol = 1e-6, double shift = 0.0) {
    ...
    if (has_rhs) {
      if (perform_inplace) {
        if (msg_lvl)
          printf("Inplace factorization cannot be used with the solver. Please "
                 "try again without -inplace.\n");
      } else {
        ...
        if (solve_type == solver_type::FULL) {
          ...
          if (solve_type == solver_type::MINRES) {
            ...
            // solve the equilibrated, preconditioned, and permuted linear
            // system
  ...
  /*! \brief Applies minres on A, preconditioning with factors L and D.
     ...
     as the residual goes below stop_tol. \param shift shifts A by
     shift*(identity matrix) to make it more positive definite. This sometimes
     helps.
  */
